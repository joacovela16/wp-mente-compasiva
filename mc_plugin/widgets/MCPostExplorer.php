...
class MCPostExplorer extends WP_Widget
{
    ...
    public function buildTree($parent, array $nodes, $field)
    {
        ...
        foreach ($elems as $item) {
            ...
            <div>
                <div>
                    <input type="checkbox" name="fieldName+'['+index+'][<?= $field ?>]'">
                    <label><?= $item->name ?></label>
                </div>
                <div class="pl-3">
                    <?php $this->buildTree($item->term_id, $children, $field) ?>
                </div>
            </div>
            <?php
        }
...
//        wp_list_categories()
        $a = 1;
        ?>
        <div class="space-y-3" x-data="$store.explorer_widget_editor(<?= $selectionAsJson ?>, '<?= $fieldName ?>')">
            <div class="space-y-5">
                <template x-for="(selection, index) in selections">
                    <template v-if="!selection.hidden">
                        <div class="space-y-5 border-1 rounded p-2">
                            <input
                                    type="checkbox"
